...
function renderRentTable() {
   ...
   const supabaseUrl = 'https://zeadgtkzqooiswyyuozl.supabase.co';
const supabaseKey = 'sb_publishable_b4jLu7Bx2dsGtLR72i8dMA_OeGcOu79'; // Publishable Key
const _supabase = supabase.createClient(supabaseUrl, supabaseKey);

// Global Database Initialization (Ek hi dafa check)
if (!window.db) window.db = JSON.parse(localStorage.getItem('krt_erp_data')) || {};
if (!db.in) db.in = [];
if (!db.out) db.out = [];
if (!db.ledgers) db.ledgers = {}; 
if (!db.opening_balances) db.opening_balances = {};
...
async function fetchCloudData() {
    try {
        ...
        // Supabase se sara data mangwao (KRT table se - Sahi Date order mein)
        const { data, error } = await _supabase
            .from('KRT')
            .select('*')
            .order('Date', { ascending: true }); 
        ...
        if (data) {
            ...
            data.forEach(row => {
                ...
                let formattedRow = {
                    ...
                    qty: stockIn > 0 ? stockIn : stockOut,
                    price: itemPrice,
                    total: (stockIn > 0 ? stockIn : stockOut) * itemPrice,
                    vendor: row.vendor_name || "factory",
                    cust: row.customer_name || "General Sale"
                };
            ...
            // Local arrays update (Ledgers safe rahenge)
            db.in = cloudIn;
            db.out = cloudOut;
...
async function addIn() {
    ...
    try {
        const { data, error } = await _supabase.from('KRT').insert([
            { Date: date, item_name: item, stock_in: qty, stock_out: 0, price: price, vendor_name: vendor }
        ]).select();
        ...
        if (data && data.length > 0) {
            db.in.push({ 
                id: data[0].id, date: date, vendor: vendor, item: item, barcode: barcode, qty: qty, price: price, total: qty * price 
            });
            saveAndRefresh();
            document.getElementById('in-item').value = "";
            document.getElementById('in-qty').value = "";
...
async function addOut() {
    ...
    const tin = db.in.filter(x => x.item === item).reduce((s, x) => s + Number(x.qty || 0), 0);
    const tout = db.out.filter(x => x.item === item).reduce((s, x) => s + Number(x.qty || 0), 0);
    const availableStock = tin - tout;
    ...
    try {
        const { data, error } = await _supabase.from('KRT').insert([
            { Date: date, item_name: item, stock_in: 0, stock_out: qty, price: price, customer_name: custName }
        ]).select();
        ...
        if (data && data.length > 0) {
            db.out.push({ 
                id: data[0].id, item: item, qty: qty, date: date, cust: custName, price: price, total: qty * price 
            });
            saveAndRefresh(); 
            document.getElementById('out-qty').value = "";
            if(document.getElementById('out-customer')) document.getElementById('out-customer').value = "";
            if(document.getElementById('stock-status')) document.getElementById('stock-status').innerText = ""; 
            alert("Stock OUT Cloud aur Local dono jagah save ho gaya!");
        }
    } catch (err) {
        alert("Internet ya Server ka masla hai!");
...
function showLiveStock(itemName) {
    ...
    let totalIn = db.in.filter(x => x.item === itemName).reduce((sum, x) => sum + Number(x.qty || 0), 0);
    let totalOut = db.out.filter(x => x.item === itemName).reduce((sum, x) => sum + Number(x.qty || 0), 0);
    let balance = totalIn - totalOut;
...
function renderAll() {
    ...
    if(todayInBody) {
        ...
        db.in.forEach((x, originalIndex) => {
            if (x.date === today) {
                htmlIn += `<tr>
                    <td>${counterIn++}</td><td>${x.item}</td><td>${x.vendor}</td><td>${x.qty}</td>
                    <td>${Number(x.price).toLocaleString()}</td><td>${Number(x.total).toLocaleString()}</td>
                    <td><button onclick="deleteEntry('in', ${originalIndex})" style="background:#e74c3c; color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer;">🗑 Del</button></td>
                </tr>`;
            }
        });
    ...
    if(outTableBody) {
        ...
        db.out.forEach((x, originalIndex) => {
            if (x.date === today) {
                htmlOut += `<tr>
                    <td>${counterOut++}</td><td>${x.date}</td><td>${x.cust}</td><td>${x.item}</td><td>${x.bc || '0'}</td><td>${x.qty}</td>
                    <td>${Number(x.price).toLocaleString()}</td><td>${Number(x.total).toLocaleString()}</td>
                    <td><button onclick="deleteEntry('out', ${originalIndex})" style="background:#e74c3c; color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer;">Del</button></td>
                </tr>`;
            }
    ...
    if(balTableBody) {
        ...
        balTableBody.innerHTML = uniqueItems.map(name => {
            if(!name) return "";
            const tin = db.in.filter(x => x.item === name).reduce((s, x) => s + Number(x.qty || 0), 0);
            const tout = db.out.filter(x => x.item === name).reduce((s, x) => s + Number(x.qty || 0), 0);
            const pPrice = Number(db.in.find(x => x.item === name)?.price || 0);
            const sPrice = Number(db.out.find(x => x.item === name)?.price || 0);
            const remainingStock = tin - tout;
            
            return `<tr>
                <td>${db.in.find(x => x.item === name)?.barcode || 'N/A'}</td>
                <td style="font-weight:bold;">${name}</td><td style="color:blue;">${tin}</td><td style="color:orange;">${tout}</td>
                <td style="font-weight:bold; color:${remainingStock < 5 ? 'red' : 'green'};">${remainingStock}</td>
                <td style="color:green; font-weight:bold;">${((sPrice - pPrice) * tout).toLocaleString()}</td>
            </tr>`;
        }).join('');
...
function generateCustomReport() {
    ...
    document.querySelector("#rep-in-table tbody").innerHTML = filteredIn.map(x => `<tr><td>${x.date}</td><td>${x.item}</td><td>${x.vendor}</td><td>${x.qty}</td><td>${x.price}</td><td>${x.total}</td></tr>`).join('') || "<tr><td colspan='6'>No Record</td></tr>";
    document.querySelector("#rep-out-table tbody").innerHTML = filteredOut.map(x => `<tr><td>${x.date}</td><td>${x.item}</td><td>${x.cust}</td><td>${x.qty}</td><td>${x.price}</td><td>${x.total}</td></tr>`).join('') || "<tr><td colspan='6'>No Record</td></tr>";

    const totalIn = filteredIn.reduce((s, x) => s + Number(x.total || 0), 0);
    const totalOut = filteredOut.reduce((s, x) => s + Number(x.total || 0), 0);
    document.getElementById('sum-in').innerText = "PKR " + totalIn.toLocaleString();
    document.getElementById('sum-out').innerText = "PKR " + totalOut.toLocaleString();
    document.getElementById('sum-profit').innerText = "PKR " + (totalOut - totalIn).toLocaleString();
}

// --- MASTER SEARCH & DELETE/EDIT ---
function generateMasterSearch() {
    const from = document.getElementById('master-from').value;
    const to = document.getElementById('master-to').value;
    if(!from || !to) { alert("Pehle Dates select karein!"); return; }
    ...
    document.querySelector("#master-in-table tbody").innerHTML = filteredIn.map((x) => {
        const originalIndex = db.in.indexOf(x); 
        return `<tr><td>${x.date}</td><td>${x.item}</td><td>${x.vendor}</td><td>${x.qty}</td><td>${x.price}</td><td>${x.total}</td><td><button onclick="editEntry('in', ${originalIndex})" style="background:#3498db; color:white; border:none; padding:5px 10px; border-radius:3px;">Edit</button> <button onclick="deleteEntry('in', ${originalIndex})" style="background:#e74c3c; color:white; border:none; padding:5px 10px; border-radius:3px;">Del</button></td></tr>`;
    }).join('') || "<tr><td colspan='7'>No Record</td></tr>";

    document.querySelector("#master-out-table tbody").innerHTML = filteredOut.map((x) => {
        const originalIndex = db.out.indexOf(x);
        return `<tr><td>${x.date}</td><td>${x.item}</td><td>${x.cust}</td><td>${x.qty}</td><td>${x.price}</td><td>${x.total}</td><td><button onclick="editEntry('out', ${originalIndex})" style="background:#3498db; color:white; border:none; padding:5px 10px; border-radius:3px;">Edit</button> <button onclick="deleteEntry('out', ${originalIndex})" style="background:#e74c3c; color:white; border:none; padding:5px 10px; border-radius:3px;">Del</button></td></tr>`;
    }).join('') || "<tr><td colspan='7'>No Record</td></tr>";
}
    ...
    if(confirm("Bilal Bhai, kya aap waqai ye record delete karna chahte hain?")) {
        ...
        if (record && record.id) {
            try {
                ...
                if (error) { alert("Cloud se delete fail: " + error.message); return; }
            } catch (err) { alert("Internet error, cloud se delete nahi ho saka."); return; }
        }
        db[type].splice(index, 1);
        saveAndRefresh(); 
        if(document.getElementById('master-in-table')) generateMasterSearch(); 
        alert("Record laptop aur cloud dono se delete ho gaya!");
    }
}

function editEntry(type, index) {
    const data = db[type][index];
    const newQty = prompt(`Nayi Quantity (Purani: ${data.qty}):`, data.qty);
    if (newQty === null) return; 
    const newPrice = prompt(`Nayi Price (Purani: ${data.price}):`, data.price);
...
// --- MULTI-USER DYNAMIC SYSTEM ---
function applyDynamicPermissions(user) {
    const menuItems = document.querySelectorAll('#sidebar ul li');
    menuItems.forEach(item => {
        let onclickAttr = item.getAttribute('onclick') || "";
        if(onclickAttr.includes('page-dashboard')) { item.style.display = "block"; return; }
        let isAllowed = user.perms.some(p => onclickAttr.includes(p));
        item.style.display = isAllowed ? "block" : "none";
    });
    renderAll();
...
function createNewUser() {
    ...
    let pass = document.getElementById('new-password').value;
    let selectedPerms = [];
    document.querySelectorAll('.perm:checked').forEach(cb => selectedPerms.push(cb.value));

    if(!name || !id || !pass) { alert("Bilal Bhai, saari details bharein!"); return; }
    extraUsers.push({ id: id, pass: pass, name: name, perms: selectedPerms });
    localStorage.setItem('krt_extra_users', JSON.stringify(extraUsers));
    alert("Naya Account Ban Gaya!");
    loadUserTable(); 
...
function loadUserTable() {
    let tbody = document.getElementById('user-table-body');
    if(!tbody) return;
    tbody.innerHTML = extraUsers.map((u, index) => `<tr><td>${u.id}</td><td>${u.name}</td><td><small>${u.perms.join(', ')}</small></td><td><button onclick="deleteExtraUser(${index})" style="background:red; color:white; border:none; border-radius:3px; cursor:pointer;">Del</button></td></tr>`).join('');
}
...
function showLedger() {
    ...
    const opening = parseFloat(db.opening_balances[name]) || 0;
    if(document.getElementById('opening-bal')) document.getElementById('opening-bal').value = opening;
    
    tbody.innerHTML = "";
    if (!name || !db.ledgers[name]) {
        ['total-ctn', 'total-debit', 'total-credit'].forEach(id => { if(document.getElementById(id)) document.getElementById(id).innerText="0"; });
        if(document.getElementById('final-balance')) document.getElementById('final-balance').innerText = "Kul Udhaar: 0";
        return;
    }

    let tCtn = 0, tDebit = 0, tCredit = 0;
    tbody.innerHTML = db.ledgers[name].map((x, index) => {
        tCtn += Number(x.ctn || 0); tDebit += Number(x.debit || 0); tCredit += Number(x.credit || 0);
        return `<tr><td>${index + 1}</td><td>${x.date}</td><td>${x.item}</td><td>${x.ctn}</td><td>${Number(x.debit).toLocaleString()}</td><td>${Number(x.credit).toLocaleString()}</td><td>${x.method}</td><td><button onclick="editLedger('${name}', ${index})" style="background:#3498db; color:white; border:none; padding:3px 7px; border-radius:3px; cursor:pointer;">Edit</button> <button onclick="delLedger('${name}', ${index})" style="background:#e74c3c; color:white; border:none; padding:3px 7px; border-radius:3px; cursor:pointer;">Del</button></td></tr>`;
    }).join('');

    document.getElementById('total-ctn').innerText = tCtn;
    document.getElementById('total-debit').innerText = tDebit.toLocaleString();
    document.getElementById('total-credit').innerText = tCredit.toLocaleString();
    document.getElementById('final-balance').innerText = "Kul Udhaar: " + ((opening + tDebit) - tCredit).toLocaleString();
}

function delLedger(custName, index) {
    if(confirm("Kya ye entry delete kar dein?")) { db.ledgers[custName].splice(index, 1); saveAndRefresh(); showLedger(); }
}
...
function editLedger(custName, index) {
    ...
    let nDebit = prompt("Naya Debit (Udhaar):", entry.debit);
    let nCredit = prompt("Naya Credit (Wasuli):", entry.credit);
    if(nDebit !== null && nCredit !== null) { db.ledgers[custName][index].debit = Number(nDebit); db.ledgers[custName][index].credit = Number(nCredit); saveAndRefresh(); showLedger(); }
}

// --- MARKET RENT BOOK SYSTEM ---
function addRentEntry() {
    const nameInput = document.getElementById('rent-name').value.trim();
    const shopInput = document.getElementById('rent-shop-no').value;
    const dateInput = document.getElementById('rent-date').value;
    ...
    if(!nameInput || !dateInput) { alert("Bilal Bhai, Customer ka Naam aur Date lazmi likhain!"); return; }

    dbRent.push({ name: nameInput, shop: shopInput, date: dateInput, month: monthInput, debit: debitInput, credit: creditInput, method: methodInput });
    localStorage.setItem('krt_rent_data', JSON.stringify(dbRent));
    alert(nameInput + " ki entry save ho gayi!");
    renderRentTable(); 
}
...
function renderRentTable() {
    ...
    if(filtered.length > 0) {
        tbody.innerHTML = filtered.map((r, index) => {
            ...
            // Original list ka asli index nikalte hain sahi se delete karne ke liye
            const originalRentIndex = dbRent.indexOf(r);
            return `<tr><td>${r.shop}</td><td>${r.date}</td><td>${r.month}</td><td style="color:red; font-weight:bold;">${r.debit.toLocaleString()}</td><td style="color:green; font-weight:bold;">${r.credit.toLocaleString()}</td><td>${r.method}</td><td><button onclick="deleteRent(${originalRentIndex})" style="background:red; color:white; border:none; padding:2px 8px; border-radius:3px; cursor:pointer;">Del</button></td></tr>`;
        }).join('');
    } else {
...
// Rent record delete karne ka naya functional core
function deleteRent(index) {
    if(confirm("Kya aap rent ki ye entry waqai delete karna chahte hain?")) {
        dbRent.splice(index, 1);
        localStorage.setItem('krt_rent_data', JSON.stringify(dbRent));
        renderRentTable();
        alert("Rent entry delete ho gayi!");
    }
}

// Live Search Trigger bind
if(document.getElementById('rent-name')) {
