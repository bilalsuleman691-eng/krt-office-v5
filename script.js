...
async function editEntry(type, index) {
    ...
    if (record.id && !record.id.toString().startsWith('local_') && _supabase && isSupabaseConnected && navigator.onLine) {
        try { await _supabase.from('KRT').update({ stock_in: type === 'in' ? qtyNum : 0, stock_out: type === 'out' ? qtyNum : 0, price: priceNum }).eq('id', record.id); } catch (err) { addPendingSync({ type: 'update', table: 'KRT', id: record.id, data: { stock_in: type === 'in' ? qtyNum : 0, stock_out: type === 'out' ? qtyNum : 0, price: priceNum } }); }
    }
...
(function() {
    ...
    if (text) text.textContent = '🐘 KRT TRADERS ERP';
    if (bar) {
        let progress = 0;
